...
function DynamicSlider2({heading, images}) {
  ...
  // The clicked slide whose big image is shown in the popup (null = closed).
  const [lightbox, setLightbox] = useState(null);

  const baseSlides =
    Array.isArray(images) && images.length > 0
      ? images.map((item, index) => {
          const small = typeof item === "string" ? item : item?.small;
          const big = typeof item === "string" ? item : item?.big ?? item?.small;
          return {
            small,
            big,
      ...
      : SLIDES.map((slide) => ({ small: slide.src, big: slide.src, label: slide.label }));
  ...
  // While the popup is open: freeze the marquee, close on Escape and lock the
  // page scroll. Resuming happens in cleanup when the popup closes.
  useEffect(() => {
    if (!lightbox) return;
        ...
        >
          {loopSlides.map((slide, index) => (
            <button
              ...
              className="group relative shrink-0 cursor-pointer overflow-hidden bg-[#EFEDE8] [backface-visibility:hidden] [transform:translateZ(0)]"
              style={{
      ...
      {lightbox ? (
        ...
        >
          <button
            ...
            aria-label="Close"
            onClick={() => setLightbox(null)}
            className="absolute right-4 top-4 z-10 flex h-10 w-10 items-center justify-center rounded-full bg-white/90 text-[#1a1a1a] shadow-sm transition hover:bg-white"
          >
            <svg
              viewBox="0 0 24 24"
              fill="none"
