...
const redirectsPath = path.join(
  process.cwd(),
  "data/service-short-slug-redirects.json"
);
...
async function run() {
  ...
  const removed = stripOrphanServiceUrlsFromSitemap0(
    redirectCount > 0 ? redirects : require("../data/service-short-slug-redirects.json")
  );
  ...
  const imagesSitemapPath = path.join(publicDir, "images-sitemap.xml");
  if (!fs.existsSync(imagesSitemapPath)) {
    fs.writeFileSync(
      imagesSitemapPath,
...
`,
    ...
    console.log("[finalize-sitemaps] Wrote empty images-sitemap.xml");
  }
