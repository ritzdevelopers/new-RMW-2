...
// Build-scoped cache-buster: defeats any persisted .next/cache fetch-cache
// entry that CI might restore across builds, without introducing
// per-request dynamism (still resolves once at build time, so it is safe
// for output: "export" and will never trigger NEXT_STATIC_GEN_BAILOUT).
const BUILD_TAG =
...
async function fetchRawBlogPages(apiUrlBuilder, fetchOptions) {
  const allBlogs = [];
